...
class NoisyLibriDataset(Dataset):
    def __init__(self, bucket_size, path, noise_type=None, noise_level=None, ascending=False):
        ...
        #print("path:", os.path.join(path, f"libri_test_noise_{noise_snr}/{noise_type}"))
        print(noise_type, noise_snr)
        if noise_type:
            snr_string = f"{noise_snr}.0" if int(noise_snr) in [10, 5, 0, -5, -10] else ""
            print("path:", os.path.join(path, f"libri_test_noise_{noise_snr}/{noise_type}"))
            file_list = sorted(list(Path(os.path.join(path, f"libri_test_noise_{snr_string}/{noise_type}")).rglob("*.wav")))
