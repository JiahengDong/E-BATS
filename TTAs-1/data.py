...
def create_dataset(split, name, path, batch_size=1, noise_type=None, noise_level=None):
    ...
    elif name.lower() == "libriadapt":
        from corpus.libriadapt import LibriAdaptDataset as Dataset
    ...
    if name.lower() == "noisylibrispeech":
...
def load_dataset(split=None, name='librispeech', path=None, batch_size=1, extra_noise=0.,noise_type=None, noise_level=None, num_workers=4):
    ''' Prepare dataloader for training/validation'''
    dataset, loader_bs = create_dataset(split, name, path, batch_size, noise_type, noise_level)
...
'''def load_dataset_extra_noise(split=None, name='noisylibrispeech', path=None, batch_size=1, noise_type=None, noise_level=None, num_workers=4):
    from corpus.librispeech import NoisyLibriDataset as Dataset
    dataset = Dataset(batch_size, path, noise_type=noise_type, noise_level=noise_level)
    loader_bs = batch_size
    collate_fn = partial(collect_audio_batch, extra_noise=0.0)
