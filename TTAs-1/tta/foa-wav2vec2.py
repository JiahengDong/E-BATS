...
class Wav2Vec2WithPrompts(nn.Module):
    def __init__(self, model, num_prompts, processor):
        ...
        if hasattr(self.model, 'hubert'):
            ...
            print("prompt_dim: ", self.prompt_dim)
        else:
    ...
    def compute_in_domain_statistics(self):
        ...
        for i, batch in enumerate(dataset):
            ...
            with torch.no_grad():
                ...
                processed_hidden_states = torch.cat([hidden_state.mean(dim=1) for hidden_state in hidden_states], dim=1) 
        ...
        all_hidden_states = accumulated_hidden_states / num_batches 
    ...
    def forward(self, input_values):
        ...
        hidden = self.base_model.encoder.layer_norm(hidden)
        ...
        # Get all the hidden states and the final hidden state representation
        final_hidden_state = hidden_states[:, self.num_prompts:, :]
...
class FOAStrategy(IStrategy):
    ...
    def forward_and_get_loss(self, x, shift_vector):
        ...
        # Calculate the discrepancy loss after shifting
        discrepancy_loss = 0.2 * (std_mse.sum() + mean_mse.sum())/64    
        # Filter non-blank tokens
        predicted_ids = torch.argmax(outputs, dim=-1) 
        non_blank = torch.where(predicted_ids != 0, 1, 0).bool()
        
        # Calculate entropy only for non-blank tokens
        entropys = softmax_entropy(outputs/2.5) 
        print("entropy_loss: ", entropys.mean(0).mean())
