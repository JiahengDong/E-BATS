...
def get_wav2vec2_featurer(model):
    """Extract the feature part of Hubert model"""
...
class T3AStrategy(IStrategy):
    """
    Test Time Template Adjustments (T3A) adapted for Hubert models
    ...
    def forward(self, input_values, attention_mask=None, adapt=True):
        ...
        non_blank_mask = (predicted_ids != 33) 
        ...
        if non_blank_mask.any():  # Only adapt if we have non-blank predictions
            ...
            # Instead of adding each frame as a support vector, compute class averages
            class_features = torch.zeros(self.num_classes, hidden_dim, device=non_blank_features.device)
            class_counts = torch.zeros(self.num_classes, device=non_blank_features.device)
            class_ent = torch.zeros(self.num_classes, device=non_blank_features.device)
            
            # Get predicted class for each non-blank frame
            non_blank_pred_ids = torch.argmax(non_blank_labels, dim=1)
            
            # Calculate average features and entropy for each class
            for c in range(self.num_classes):
                class_mask = (non_blank_pred_ids == c)
                if class_mask.sum() > 0:
                    class_features[c] = non_blank_features[class_mask].mean(dim=0)
                    class_ent[c] = non_blank_ent[class_mask].mean()
                    class_counts[c] = class_mask.sum()
            
            # Add only classes that have non-blank predictions
            active_classes = (class_counts > 0).nonzero().squeeze(-1)
            
            self.supports = self.supports.to(class_features.device)
            self.labels = self.labels.to(class_features.device)
            self.ent = self.ent.to(class_features.device)
            
            # Create one-hot labels for the averaged class features
            avg_labels = torch.zeros(len(active_classes), self.num_classes, device=class_features.device)
            for i, c in enumerate(active_classes):
                avg_labels[i, c] = 1.0
            
            # Append only the averaged features for classes that have samples
            if len(active_classes) > 0:
                self.supports = torch.cat([self.supports, class_features[active_classes]])
                self.labels = torch.cat([self.labels, avg_labels])
                self.ent = torch.cat([self.ent, class_ent[active_classes]])
